...
app.post('/api/burn', upload.fields([{ name: 'video', maxCount: 1 }]), (req, res) => {
  ...
  // Build vf filter — fontsdir tells libass where Liberation/DejaVu fonts are
  const subsFilter = `subtitles='${assPath.replace(/'/g,"\\'")}':fontsdir='${FONTS_DIR}'`;
  ...
  execFile(FFMPEG, args, { maxBuffer: 500 * 1024 * 1024 }, (err, stdout, stderr) => {
    ...
    if (err) {
      ...
      return res.status(500).json({ error: 'FFmpeg failed: ' + (stderr ? stderr.slice(-300) : err.message) });
    }
